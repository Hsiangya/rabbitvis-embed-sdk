import { EMBED_HOST_SOURCE, EMBED_PROTOCOL_VERSION, parseEmbedEvent, } from './protocol.js';
import { requestEmbedSession, } from './session-provider.js';
...
function prepareEmbedUrl(session, expectedOrigin, instanceId) {
    const url = new URL(session.embedUrl);
    if (url.origin !== expectedOrigin)
        throw new Error('embedUrl origin does not match rabbitVisOrigin');
    ...
    if (!/^\/embed\/?$/.test(url.pathname))
        throw new Error('embedUrl must target /embed/');
    if (url.search)
        throw new Error('embedUrl must use the fixed /embed/#code=... shape');
    const fragment = new URLSearchParams(url.hash.replace(/^#/, ''));
    const launchCode = fragment.get('code');
    if (!launchCode)
        throw new Error('embedUrl is missing its launch code');
...
export class RabbitVisEmbed {
    ...
    iframe;
    expectedOrigin;
    currentInstanceId = randomInstanceId();
    destroyed = false;
    reloadInFlight = null;
    get instanceId() {
    ...
    async mount() {
        if (this.destroyed)
            throw new Error('RabbitVisEmbed has been destroyed');
    ...
    reloadSession() {
        if (this.destroyed)
            return Promise.reject(new Error('RabbitVisEmbed has been destroyed'));
    ...
    async loadFreshSession() {
        ...
        if (this.destroyed || loadInstanceId !== this.currentInstanceId)
            return;
    ...
    onMessage = (event) => {
        if (this.destroyed || event.origin !== this.expectedOrigin)
            return;
...
export async function mountRabbitVisEmbed(options) {
