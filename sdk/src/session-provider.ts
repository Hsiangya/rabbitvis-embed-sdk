...
function parseSession(value: unknown): RabbitVisEmbedSession {
...
export async function requestEmbedSession(provider: RabbitVisSessionProvider): Promise<RabbitVisEmbedSession> {
  ...
  if (!response.ok) throw new Error(`Partner session endpoint failed (${response.status})`)
  const text = await response.text()
  if (text.length > 64 * 1024) throw new Error('Partner session response is too large')
